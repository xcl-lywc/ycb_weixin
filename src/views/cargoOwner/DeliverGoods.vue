			...
			*/
			...
			publishGoodsSource(){
				...
				if(this.addOrder.cargo_images==''){
					this.$toast("请上传货物实物照片")
					return;
				}
				
				this.addOrder.payer = Number(this.payer);
				this.axios.post(`${this.commonJs.basePath}/v1/order/add`,this.addOrder).then( (response) => {
					this.$toast('发货成功')
					this.$router.push({name:'matchList',params:{id:response.data.data.id}})
					// this.$router.push({path: '/deliver_goods/match_list'})
				}).catch( (error) => {
					this.$toast(error)
				});
			},
